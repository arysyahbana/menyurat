...
@section('content')
    ...
    <div class="mt-3">
        <div style="text-align: center">
            <h4><u>PERJANJIAN KERJA WAKTU TAK TERTENTU (PKWTT)</u></h4>
                ...
                @if (old("sections"))
                    @foreach (old("sections") as $index => $item)
                        <div id="{{ sprintf("sections_%s_data", $index) }}">
                            <p style="font-weight: bold; text-align: center;">Pasal {{ $loop->iteration }}</p>
                            <p style="margin-top: -20px; font-weight: bold; text-align: center;" id="{{ sprintf('sections_%s_title_data', $index) }}">{{ $item["title"] }}</p>
                            <div style="margin-top: -20px" id="{{ sprintf('sections_%s_content_data', $index) }}">
                                {!! $item["content"] !!}
                            </div>
                        </div>
                    @endforeach
                ...
                @else
                    @foreach ($logs["sections"] as $index => $item)
                        <div id="{{ sprintf("sections_%s_data", $index) }}">
                            <p style="font-weight: bold; text-align: center;">Pasal {{ $loop->iteration }}</p>
                            <p style="margin-top: -20px; font-weight: bold; text-align: center;" id="{{ sprintf('sections_%s_title_data', $index) }}">{{ $item["title"] }}</p>
                            <div style="margin-top: -25px" id="{{ sprintf('sections_%s_content_data', $index) }}">
                                {!! $item["content"] !!}
                            </div>
                        </div>
                    @endforeach
