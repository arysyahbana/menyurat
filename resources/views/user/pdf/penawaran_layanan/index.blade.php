...
@section('title', 'Penawaran Layanan')
    ...
    <table width="100%" style="font-size: 11pt">
        ...
        <tr>
            <td colspan="2"  style="padding-left: 5px">
                ...
                <ol style="margin-top: 0px">
                    @if (old("contents.second"))
                        @foreach (old("contents.second") as $index => $item)
                            <li style="margin-top: -15px" id="{{ sprintf('contents_second_%s_key_data', $index) }}">
                                <p style="font-weight: bold">{{ $item["key"] }}</p>
                                <p style="margin-top: -10px" id="{{ sprintf('contents_second_%s_value_data', $index) }}">{{ $item["value"] }}</p>
                            </li>
                        @endforeach
                ...
                <p style="text-indent: 30px; margin-top: -15px;">&emsp;&emsp;&emsp;<span id="contents_third_data">{{ old("contents.third") ?? $logs["contents"]["third"] }}</span></p>
            </td>
        </tr>
        <tr>
